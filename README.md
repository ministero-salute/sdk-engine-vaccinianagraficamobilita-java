...
All’interno delle specifica del tracciato sono indicati i dettagli dei campi di business del tracciato di input atteso da SDK, il quale differisce per i sei  flussi dell’area AVN. All’interno di tale file è presente la colonna **Posizione nel file** la quale rappresenta l’ordinamento delle colonne del tracciato di input da caricare all’SDK.
...
| :- | :- | :- | :-: | :-: | :-: | :-: | :-: | :-: |
|Modalita|0| |Campo tecnico utilizzato per distinguere le modalità di invio di: <br>schede vaccinali di soggetti trasferiti ad altra regione per cambio di residenza <br>oppure <br>singole vaccinazioni effettuate da soggetti non residenti in regione.|A|OBB|Deve essere utilizzato il codice “TR” per la trasmissione delle informazioni anagrafiche relative alla scheda vaccinale dei soggetti trasferiti in  altra regione per un cambio di residenza,  rispetto alla regione che sta trasmettendo.<br>Deve essere utilizzato il codice “MV” per la trasmissione delle informazioni anagrafiche relative ad eventi vaccinali effettuati a soggetti non  residenti (domiciliati sanitari, eventi occasionali) nella regione che sta trasmettendo. |2|**/informazioniAnagrafiche/@Modalita**|
|Codice Regione|1|KEY|Individua la Regione che trasmette il dato. |AN|OBB|Il codice da utilizzare è quello a tre caratteri definito con DM 17 settembre 1986, pubblicato nella Gazzetta Ufficiale n.240 del 15 ottobre 1986, e successive modifiche, utilizzato anche nei modelli per le rilevazioni delle attività gestionali ed economiche delle Aziende unità sanitarie locali.I valori ammessi sono quelli riportati all’**Allegato 1 – Regioni**|3|**/informazioniAnagrafiche/@CodiceRegione**|
|Codice Identificatico dell’Assistito|2|KEY|Codice identificativo dell’assistito|AN|OBB|Il campo deve avere lunghezza massima di 20 caratteri in input alla procedura di cifratura che produrrà un output di massimo 172 caratteri.<br>Le modalità di alimentazione del presente campo sono descritte nel paragarfo 3.6 Codice Identificativo dell’ Assistito – procedura di cifratura|172|**/informazioniAnagrafiche/Assistito/IdAssistito**|
...
|Comune di  Residenza|7| |Identifica il Comune nella cui anagrafe è iscritto l’assistito.|AN|FAC|Formato: NNNNNN<br>Il codice da utilizzare è il codice secondo codifica ISTAT, i cui primi tre caratteri individuano la provincia e i successivi un progressivo all’interno di ciascuna provincia che individua il singolo comune.|6|**/informazioniAnagrafiche/Assistito/ComuneResidenza**|
...
Di seguito sono indicati i controlli da configurare sulla componente di Validation Engine e rispettivi error code associati riscontrabili sui dati di input per il flusso AVM.
...
| :-: | :-: | :-: | :-: | :-: | :-: | :-: | :-: | :-: | :-: |
|Modalità|AVM|1801|ATTIVA|Mancata valorizzazione di un campo obbligatorio|Tag XML non presente o tag XML presente ma non valorizzato.| | |scarti|Basic|
|Modalità|AVM|1802|ATTIVA|Non appartenenza al dominio di riferimento|Valori diversi da quelli ammessi : TR, MV| | |scarti|Basic|
|Modalità|AVM|1803|ATTIVA|Modalità non coerente con il flusso inviato|in caso di flusso Trasferimenti/Mobilità il naming del flusso conterrà TM e la modalità è diversa da TR o MV| | |scarti|Basic|
|Tipo Anagrafica|AVM|1901|ATTIVA|Mancata valorizzazione di un campo obbligatorio|Tag XML non presente o tag XML presente ma non valorizzato.| | |scarti|Basic|
|Tipo Anagrafica|AVM|1902|ATTIVA|Non appartenenza al dominio di riferimento|Valori diversi da quelli ammessi : I,i,V,v,C,c| | |scarti|Basic|
|Codice Regione |AVM|1921|ATTIVA|Mancata valorizzazione di un campo obbligatorio|Tag XML non presente o tag XML presente ma non valorizzato.| | |scarti|Basic|
|Codice Regione |AVM|1922|ATTIVA|Non appartenenza al dominio di riferimento|Il valore inserito e controllato non è presente in anagrafica regioni|Anagrafiche: REGIONE| |scarti|Anagrafica|
|Codice Regione |AVM|1905|ATTIVA|Il codice regione non coincide con la regione inviante.|Il campo Codice Regione non coincide con la regione che sta trasmettendo il file. | | |scarti|Basic|
|Codice Identificatico dell’Assistito|AVM|1701|ATTIVA|Mancata valorizzazione di un campo obbligatorio|Tag XML non presente o tag XML presente ma non valorizzato.| | |scarti|Basic|
|Codice Identificatico dell’Assistito|AVM|1703|ATTIVA|Lunghezza diversa da quella attesa|La lunghezza è diversa da 172 caratteri| | |scarti|Basic|
|Validità codice Identificativo|AVM|1701|ATTIVA|Mancata valorizzazione di un campo obbligatorio|Tag XML non presente o tag XML presente ma non valorizzato.| | |scarti|Basic|
...
|Data Trasferimento Residenza|AVM|6170|ATTIVA|Datatype errato|Il campo deve essere valorizzato con il formato data AAAA-MM-GG| | |scarti|Basic|
...
La DATA\_COMPETENZA da utilizzare per filtrare il dato anagrafico per il flusso AVM sulle tabelle con date di validità specifiche, riportate in precedenza, è l’Anno Riferimento passato in input all’SDK.
